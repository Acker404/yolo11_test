    ...
    QObject::connect(ui.Button_openImage, &QPushButton::clicked, this, [=]() {  
        ...
        if (!fileName.isEmpty()) {  
            ...
            for (int i = 0; i < detections; ++i) // Change 'detections.size()' to 'detections' since 'detections' is an integer
            {
                ...
                double scaleFactor = std::max(0.5, std::min(1.0, imgHeight / 720.0)); // ��ĳ��ҽd�� 0.5 ~ 1.0
                ...
                textBox &= cv::Rect(0, 0, input_bgr.cols, input_bgr.rows); // ��������W�X���
